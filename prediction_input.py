...
def build_tfrecord_input(training=True, tfrecord_format=0):
  ...
  """
  filenames = sorted(gfile.Glob(os.path.join(FLAGS.data_dir, '*')))
  print(filenames)
  if not filenames:
  ...
  if FLAGS.use_state:
    ...
    [key_batch, image_batch, action_batch, state_batch] = tf.train.batch(
        [image_seq, action_seq, state_seq],
        FLAGS.batch_size,
        num_threads=FLAGS.batch_size,
        capacity=100 * FLAGS.batch_size)
    return key_batch, image_batch, action_batch, state_batch
  else:
    ...
    return key_batch, image_batch, zeros_batch, zeros_batch
